...
export const buildDefaultPropsAst = (componentName, propDefinitions, t = types) => {
    if (!propDefinitions) {
        return null;
    }
    const defaultValuesSearch = Object.keys(propDefinitions).reduce((acc, key) => {
    ...
    const memberExpression = t.memberExpression(t.identifier(componentName), t.identifier('defaultProps'));
...
export const buildTypesOfPropsAst = (componentName, propTypesNames, propDefinitions, t = types) => {
    if (!propDefinitions) {
        return null;
    }
    const defaultValuesSearch = Object.keys(propDefinitions).reduce((acc, key) => {
    ...
    const memberExpression = t.memberExpression(t.identifier(componentName), t.identifier('propTypes'));
    const memberExpressionValue = objectToObjectExpression(defaultValuesSearch.values);
    const assignmentExpression = t.assignmentExpression('=', memberExpression, memberExpressionValue);
    return t.expressionStatement(assignmentExpression);
};
export const buildReactSFCTypescriptAnnotation = (annotationName, t = types) => {
