import { ComponentStructure, ChunkDefinition, GeneratorFunction } from '../types'
...
export default class Builder {
  ...
  public link(structure?: ComponentStructure): string {
    ...
    chunks.forEach((chunk) => {
      ...
      if (linker.after) {
        dependencies[chunk.name].after = dependencies[chunk.name].after.concat(
          linker.after
        )
    ...
    Object.keys(dependencies).forEach((key) => {
      if (!dependencies[key].chunk) {
        throw new Error(
          `chunk with name ${key} was referenced by other chunks but not found.`
        )
      }
    })
