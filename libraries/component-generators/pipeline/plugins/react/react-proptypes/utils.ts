...
export const buildDefaultPropsAst = (
  ...
  propDefinitions: { [key: string]: { type: string; defaultValue: any } },
  t = types
) => {
  if (!propDefinitions) {
    return null
...
export const buildTypesOfPropsAst = (
  ...
  const defaultValuesSearch = Object.keys(propDefinitions).reduce(
    (acc: any, key) => {
      ...
      if (defaultValue) {
        const astValue = t.memberExpression(
          t.identifier(propTypesNames),
          t.identifier(type)
        )
...
export const buildReactSFCTypescriptAnnotation = (annotationName: string, t = types) => {
  ...
  const annotation = t.genericTypeAnnotation(
    t.identifier('React.SFC'),
    ComponentNameInBraketsPart
  )
